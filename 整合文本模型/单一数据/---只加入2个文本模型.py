...
wenben_file='media_index.xlsx'
first_columns='media_attention'
...
def my_model(long_term_back,short_term_back,wenben_sort):
    ...
    # # 15分钟频训练结果和日频训练结果合并
    concatenated=layers.concatenate([LSTM_long_term,LSTM_short_term],axis=-1) # axis=-1按照最后一个轴粘合

    alloy=layers.Dense(20,activation='relu')(concatenated) #将粘合结果再接一个全连接层
    dropout=layers.Dropout(0.2)(alloy)
    output=layers.Dense(1,activation='sigmoid')(dropout)
    model=Model([wenben_long_term_input,wenben_short_term_input],output) #八股文：将输入和输出圈起来
...
history=model.fit(x=[wenben_long_term_train,wenben_short_term_train],y=target_train,batch_size=batch_size,validation_split=0.2,epochs=epochs)
