...
def my_model(long_term_back,short_term_back,wenben_sort):
    ...
    # 15分钟频训练结果和日频训练结果合并
    concatenated=layers.concatenate([LSTM_fif,LSTM_daily,LSTM_long_term],axis=-1) # axis=-1按照最后一个轴粘合

    alloy=layers.Dense(dense_num,activation='relu')(concatenated) #将粘合结果再接一个全连接层
    dropout=layers.Dropout(0.2)(alloy)
    output=layers.Dense(1,activation='sigmoid')(dropout)
    model=Model([fif_min_input,daily_input,wenben_long_term_input],output) #八股文：将输入和输出圈起来
...
history=model.fit(x=[fif_train,daily_train,wenben_long_term_train],y=target_train,batch_size=batch_size,validation_split=0.25,epochs=epochs)


loss,accuracy = model.evaluate([fif_test,daily_test,wenben_long_term_test],y=target_test)
print(loss,accuracy)

def gen_y_pred():
    y_predict=model.predict([fif_test,daily_test,wenben_long_term_test]).reshape(test_num-wenben_back).tolist()
    y_pred=[]
