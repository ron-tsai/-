...
df = pd.read_excel(os.path.join(path, '2021年-.xlsx'))
contents = []
...
def seg_sentence_score(list):  # 单句情感得分计算;输入单句中分句列表
    ...
    if summary_sentence_score_list!=[]:
        ...
        # 计算
        print('seg_sentence_score_list',seg_sentence_score_list)
        print(len(np.nonzero(seg_sentence_score_list)[0]))
...
df.to_excel(os.path.join(save_path, '2021年赋分.xlsx'))
