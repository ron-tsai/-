...
x2_train, x2_test,y_train, y_test = train_test_split(daily_train, target_train ,test_size=validation_split,random_state=seed,shuffle=True)
print('@@@@@@@@@',x2_train)
...
def my_model():
    ...
    # 15分钟频训练结果和日频训练结果合并
    # concatenated=layers.concatenate(LSTM_daily,axis=-1) # axis=-1按照最后一个轴粘合

    alloy=layers.Dense(dense_num,activation='relu')(LSTM_daily) #将粘合结果再接一个全连接层
...
history=model.fit(x=x2_train,y=y_train,batch_size=batch_size,epochs=epochs,validation_data=(x2_test ,y_test))
...
def gen_y_pred():
    y_predict=model.predict(daily_test,batch_size=1).reshape(test_num-wenben_back).tolist() ###batch_size要设为1，不然evaluate和predict结果不同
    y_pred=[]
