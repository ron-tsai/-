...
x2_train, x2_test, x3_train, x3_test,y_train, y_test = train_test_split(daily_train,wenben_short_term_train, target_train ,test_size=validation_split,random_state=seed,shuffle=True)
print('@@@@@@@@@',x2_train)
...
def my_model(short_term_back,wenben_sort):
    ...
    # 15分钟频训练结果和日频训练结果合并
    concatenated=layers.concatenate([LSTM_daily,LSTM_short_term],axis=-1) # axis=-1按照最后一个轴粘合

    alloy=layers.Dense(dense_num,activation='relu')(concatenated) #将粘合结果再接一个全连接层
    dropout=layers.Dropout(0.2)(alloy)
    output=layers.Dense(1,activation='sigmoid')(dropout)
    model=Model([daily_input,wenben_short_term_input],output) #八股文：将输入和输出圈起来

    print(model.summary())
    model.compile(optimizer=keras.optimizers.Adam(lr=1e-3),loss='binary_crossentropy',metrics=['acc'])
    return model
    # reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=5, mode='auto')
model=my_model(short_term_back=short_term_back,wenben_sort=wenben_sort)

plot_model(model, show_shapes=True, show_layer_names=True)

# callback = keras.callbacks.EarlyStopping(monitor='loss', patience=1)#使用loss作为监测数据，轮数设置为1

history=model.fit(x=[x2_train,x3_train],y=y_train,batch_size=batch_size,epochs=epochs,validation_data=([x2_test,x3_test] ,y_test))

# plot_model(model, show_shapes=True, show_layer_names=False)

loss,accuracy = model.evaluate([daily_test,wenben_short_term_test],y=target_test)
print(loss,accuracy)


def gen_y_pred():
    y_predict=model.predict([daily_test,wenben_short_term_test],batch_size=1).reshape(test_num-wenben_back).tolist() ###batch_size要设为1，不然evaluate和predict结果不同
    y_pred=[]
...
#ROC曲线绘制
y_predict=model.predict([daily_test,wenben_short_term_test]).reshape(test_num-wenben_back).tolist()
