    ...
    def fif_train_data(self,data):
        while True:
            ...
            for j in rows:
                ...
                samples[j] = data.loc[
                              (data.index >= 18*16+j * self.fif_back) & (data.index <18*16+ (j+1) * self.fif_back),
                              'open':]
    ...
    def fif_test_data(self,data):
        while True:
            ...
            for j in rows:
                ...
                samples[j] = data.loc[
                                 (data.index >=18*16+16*self.train_num+(j) * self.fif_back) & (data.index < 18*16+16*self.train_num+(j+1) * self.fif_back),
                                 'open':]
...
print('文本训练切分：',wenben_norm_train_df.shape)
print(wenben_norm_train_df)
wenben_norm_test_df=split_data()['wenben_test_df']
print('文本测试切分：',wenben_norm_test_df.shape)
print(wenben_norm_test_df)
...
print('文本数据',wenben_short_term_train)
...
history=model.fit(x=[fif_train,daily_train,wenben_short_term_train],y=target_train,batch_size=batch_size,validation_split=0.25,epochs=epochs)
