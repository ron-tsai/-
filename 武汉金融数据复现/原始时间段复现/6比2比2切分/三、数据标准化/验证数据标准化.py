...
daily_open_mean_value = daily_val_df['open'].mean(axis=0)
daily_high_mean_value = daily_val_df['high'].mean(axis=0)
daily_low_mean_value = daily_val_df['low'].mean()
daily_close_mean_value = daily_val_df['close'].mean()
daily_volumerate_mean_value = daily_val_df['volume_rate'].mean()

daily_open_std_value = daily_val_df['open'].std()
daily_high_std_value = daily_val_df['high'].std()
daily_low_std_value = daily_val_df['low'].std()
daily_close_std_value = daily_val_df['close'].std()
daily_volumerate_std_value = daily_val_df['volume_rate'].std()

fif_open_mean_value = fif_val_df['open'].mean(axis=0)
fif_high_mean_value = fif_val_df['high'].mean(axis=0)
fif_low_mean_value = fif_val_df['low'].mean()
fif_close_mean_value = fif_val_df['close'].mean()

fif_volumerate_mean_value = fif_val_df['volume_rate'].mean()

fif_open_std_value = fif_val_df['open'].std()
fif_high_std_value = fif_val_df['high'].std()
fif_low_std_value = fif_val_df['low'].std()
fif_close_std_value = fif_val_df['close'].std()
fif_volumerate_std_value = fif_val_df['volume_rate'].std()
def daily_norm(df):
...
fif_norm.to_excel('F:\\newstart\software\category\\tool\category\deal_with_data\武汉金融数据\武汉金融复现：大数据量12年起始\\6比2比2\标准化后数据\\fif_val_data.xlsx')
daily_norm.to_excel('F:\\newstart\software\category\\tool\category\deal_with_data\武汉金融数据\武汉金融复现：大数据量12年起始\\6比2比2\标准化后数据\\daily_val_data.xlsx')
