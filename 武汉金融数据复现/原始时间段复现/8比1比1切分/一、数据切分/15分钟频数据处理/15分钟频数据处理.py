...
total_df2=df2.loc[(df2['trade_time']>=pd.to_datetime("2012-01-05"))&(df2['trade_time']<=pd.to_datetime("2019-09-30")),:]
#新建滞后一期列
s2=df2.loc[(df2['trade_time']>=pd.to_datetime('2012-01-05 15:00:00'))&(df2['trade_time']<=pd.to_datetime('2019-09-30 14:45:00')),'volume'].tolist()
print(s2)
print(total_df2.loc[(total_df2['trade_time']>=pd.to_datetime('2012-01-06 9:45:00'))&(total_df2['trade_time']<=pd.to_datetime('2019-09-30 15:00:00')),:])
total_df2.loc[(total_df2['trade_time']>=pd.to_datetime('2012-01-06 9:45:00'))&(total_df2['trade_time']<=pd.to_datetime('2019-09-30 15:00:00')),'volume-1']=s2
...
#切割数据集
fif_train_df=total_df2.loc[(total_df2['trade_time']>=pd.to_datetime("2012-01-06"))&(total_df2['trade_time']<pd.to_datetime("2016-08-09")),:]
fif_val_df=total_df2.loc[(total_df2['trade_time']>=pd.to_datetime("2018-02-23"))&(total_df2['trade_time']<pd.to_datetime("2019-05-22")),:]
fif_test_df=total_df2.loc[(total_df2['trade_time']>=pd.to_datetime("2018-02-23"))&(total_df2['trade_time']<pd.to_datetime("2019-05-22")),:]
