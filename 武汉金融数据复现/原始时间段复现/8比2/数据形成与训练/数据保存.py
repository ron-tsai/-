...
##验证集切分
daily_val=daily_train[1076:]
daily_train=daily_train[:1076]
fif_val=fif_train[1076:]
fif_train=fif_train[:1076]
target_val=target_train[1076:]
target_train=target_train[:1076]

print(Counter(target_val))
print(daily_train.shape,daily_val.shape,daily_test.shape,fif_train.shape,fif_val.shape,fif_test.shape,target_train.shape,target_val.shape,target_test.shape)
...
# ,callbacks=[reduce_lr]
history=model.fit(x=[fif_train,daily_train],y=target_train,batch_size=32,validation_data=([fif_val,daily_val],target_val),epochs=100)
