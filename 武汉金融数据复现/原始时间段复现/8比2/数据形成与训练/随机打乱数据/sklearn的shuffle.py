...
print(daily_train.shape,fif_train.shape,target_train.shape)

ziped=zip(list(daily_train),list(fif_train))

train,val= shuffle(ziped, target_train)
...
print(model.summary())
model.compile(optimizer=keras.optimizers.adam(lr=1e-6),loss='binary_crossentropy',metrics=['acc'])


history=model.fit(x=train,y=target_train,batch_size=32,validation_data=val,epochs=100)
