...
origin_data=Data_maker(train_num=1509,test_num=373,fif_back=16,daily_back=20)

dir='F:\\newstart\software\category\\tool\category\deal_with_data\武汉金融数据\标准化处理数据基础\原始数据复现'
...
def split_data(train_num=1509,daily_back=20):
...
data=shuffle_data(fif_data=fif_train,daily_data=daily_train,
                  target=target_train,train_num=1509,
                  test_num=373,fif_back=16,daily_back=20)
...
def my_model():
    ...
    print(model.summary())
    model.compile(optimizer=keras.optimizers.adam(lr=1e-3),loss='binary_crossentropy',metrics=["acc",getRecall,getPrecision])
    return model
