...
def fif_pre(dir,start,end):
    ...
    df=pd.read_csv(os.path.join(dir,'沪深300-15min.csv'),dtype=object)
    # print(df2)
...
def daily_pre(dir,start,end):
    df=pd.read_excel(os.path.join(dir,'step1：日频原数据拼接去重.xlsx'),dtype=object,col_index=False)
    df=df.loc[:,~df.columns.str.contains('Unnamed')]
    df['trade_time']=pd.to_datetime(df['trade_time'])
    df.sort_values(by='trade_time',ascending=True,inplace=True)
    ...
    #新建滞后一期列
    s=df.loc[(df['trade_time']>=pd.to_datetime(start))&(df['trade_time']<pd.to_datetime(end)+timedelta(days=-1)),'volume'].tolist()
