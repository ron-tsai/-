...
print(total_df)
#切割数据集
daily_train_df=total_df.loc[(total_df['trade_time']>=pd.to_datetime("2012-02-27"))&(total_df['trade_time']<=pd.to_datetime("2017-06-30")),:]
daily_val_df=total_df.loc[(total_df['trade_time']>=pd.to_datetime("2017-07-03"))&(total_df['trade_time']<=pd.to_datetime("2019-05-22")),:]
daily_test_df=total_df.loc[(total_df['trade_time']>=pd.to_datetime("2019-05-23"))&(total_df['trade_time']<=pd.to_datetime("2021-04-12")),:]
save_dir='F:\\newstart\software\category\\tool\category\deal_with_data\武汉金融数据\武汉金融复现：大数据量12年起始\\6比2比2\日频数据'
print('daily_train_df:',daily_train_df,'daily_val_df:',daily_val_df,'daily_test_df:',daily_test_df,'daily_test_df:',daily_test_df.shape)
