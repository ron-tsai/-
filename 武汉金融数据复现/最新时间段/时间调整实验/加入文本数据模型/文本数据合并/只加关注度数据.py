...
origin_data=Data_maker(train_num=1650,test_num=413,fif_back=16,daily_back=20,long_term_back=15,short_term_back=5)
...
wenben_df=pd.read_excel(os.path.join(wenben_dir,'guanzhudu_data.xlsx'))
...
def my_model(long_term,short_term):
    ...
    # 文本输入训练(!!!卷积滤镜行列先后)
    wenben_long_term_input=Input(shape=(long_term,2),dtype='float32',name='wenben_long_term_input')
    Conv1D_fif=layers.Conv1D(16,1,strides=1)(wenben_long_term_input)
    LSTM_long_term=layers.LSTM(100)(Conv1D_fif)
    wenben_short_term_input = Input(shape=(short_term, 2), dtype='float32', name='wenben_short_term_input')
...
model=my_model(long_term=15,short_term=5)

history=model.fit(x=[fif_train,daily_train,wenben_long_term_train,wenben_short_term_train],y=target_train,batch_size=8,validation_split=0.2,epochs=30)
