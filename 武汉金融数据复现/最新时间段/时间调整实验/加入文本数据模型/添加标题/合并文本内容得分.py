...
df0=pd.DataFrame(columns=['date','title','sector_score'])
for file in file_list:

    df=pd.read_excel(os.path.join(path,file),dtype=object,usecols=['date','title','sector_score'])

    df0=df0.append(df,ignore_index=False)


df1=df0.loc[(df0['date']>=pd.to_datetime('2013-01-09'))&(df0['date']<=pd.to_datetime('2021-07-02')),:]
df1.to_excel(os.path.join(path,'内容得分合并.xlsx'),index=False)
# title_score_list=[]
