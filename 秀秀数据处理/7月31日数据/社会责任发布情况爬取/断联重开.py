...
path='C:\\Users\Administrator\Desktop\秀秀数据\待做\\731数据\社会责任爬取'
...
for beg in range(10,20):
    end=beg+2

    param={'pageNum': 1,'searchkey': '社会责任报告','pageSize': 30,'seDate': '20{}-01-01~20{}-12-31'.format(beg,end),'column': 'szse','tabName': 'fulltext','isHLtitle': 'true'}
    headers = {
    ...
    for page in range(1,total_num+1):
        if '{}-{}-{}'.format(page,beg,end) not in complete_list:

            param = {'pageNum': page, 'searchkey': '社会责任报告', 'pageSize': 30,
                     'seDate': '20{}-01-01~20{}-12-31'.format(beg, end), 'column': 'szse', 'tabName': 'fulltext',
                     'isHLtitle': 'true'}
            headers = {
                "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/88.0.4324.146 Safari/537.36"
