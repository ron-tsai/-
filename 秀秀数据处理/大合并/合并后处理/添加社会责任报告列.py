...
df=pd.read_excel(os.path.join(path,'排序ID.xlsx'),dtype=object)
df.loc[:,'截止日期']=pd.to_datetime(df['截止日期']).dt.date
df1=pd.read_excel(os.path.join(path1,'社会责任报告只差股票代码筛选.xlsx'),dtype=object)
...
for name, group in g:
    ...
    print(group)
    group.drop_duplicates(subset=['证券代码', '截止日期'], keep='first', inplace=True)
...
df3 = pd.merge(df0, df, on=['证券代码', '截止日期'], how='right')
...
df3.to_excel(os.path.join(path,'最终合并数据.xlsx'),index=False)
